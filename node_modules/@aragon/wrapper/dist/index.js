...
// full repo objects.
(0,_operators.concatMap)(async({repos,updatedRepoAppId:appId})=>{const{repoAddress,versions}=repos[appId],installedRepoInfo=installedRepoCache.get(appId),latestVersion=versions[versions.length-1],currentVersion=Array.from(versions)// Apply reverse to find the latest version with the currently installed contract address
.reverse().find(version=>(0,_utils.addressesEqual)(version.contractAddress,installedRepoInfo.contractAddress)),currentVersionRequest=applicationInfoCache.request("".concat(appId,".").concat(currentVersion.contractAddress)).catch(()=>({})).then(content=>({content,version:currentVersion.version})),versionInfos=await Promise.all([currentVersionRequest,(0,_utils.addressesEqual)(currentVersion.contractAddress,latestVersion.contractAddress)?currentVersionRequest// current version is also the latest, no need to refetch
:applicationInfoCache.request("".concat(appId,".").concat(latestVersion.contractAddress)).catch(()=>({})).then(content=>({content,version:latestVersion.version}))]);// Emit updated repo information
return{appId,repoAddress,versions,currentVersion:versionInfos[0],latestVersion:versionInfos[1]}}));this.installedRepos=repo$.pipe(// Finally, we reduce the merged updates from individual repos into one final, expanding array
